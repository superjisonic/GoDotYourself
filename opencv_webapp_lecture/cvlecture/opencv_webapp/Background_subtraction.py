...
Key 'a' - To execute auto remove
Key 'n' - To update the segmentation
Key 'r' - To reset the setup
Key 'o' - To turn on or turn off auto segmentation mode
...
def draw_detections(img, rects, thickness=1): # to check face recognition
...
def grabcut(img, mask, faces): # grabcut with multiple faces
    ...
    cv2.grabCut(img, mask,
                check_border(img, pad_x - w, pad_y - int(0.5*h), pad_w + 2*w, pad_h + 8*h),
                bgdmodel, fgdmodel, 1,
...
class BGsubtractor():
    ...
    auto = True  # flag for choosing auto update
    auto_update = False  # flag to update image automatically
    ...
    def run(self, path):
        ...
        if (type(self.img) is np.ndarray):
            ...
            cv2.setMouseCallback('input', self.onmouse) # for mouse input
            ...
            while (1):
                ...
                elif k == ord('o'): # turn on or turn off auto segmentation
                    self.auto = not self.auto
                    print('auto_update ', ['off', 'on'][self.auto])
                ...
                elif k == ord('n') or self.auto_update is True:  # segment the image
